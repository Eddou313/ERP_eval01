...
export async function createOrder(form: OrderForm): Promise<number> {
  const xml = buildOrderXml(form);
  const response = await requestPrestashopXml<{ prestashop: { order: { id: unknown } } }>(
    `/orders`,
...
export async function ImportOrder(form: OrderImport): Promise<number> {
  const xml = buildOrderTXml(form);
  console.debug("PrestaShop order import XML:", xml);
  const response = await requestPrestashopXml<{ prestashop: { order: { id: unknown } } }>(
    `/orders`,
    { method: "POST", bodyXml: xml }
  );
...
function buildOrderXml(form: OrderForm): string {
  ...
  if (form.gift_message) {
    (root.order as any).gift_message = {
      language: {
        "@_id": "1",
